...
with st.expander("➕ Nayi Customer Add Karein", expanded=False):
    ...
    if submitted:
        try:
            if not name.strip():
                raise ValueError("Naam likhna zaroori hai.")
            cid = create_customer(name.strip(), phone.strip() or None)
            st.success(f"{name.strip()} add ho gaya/gayi. ID: {cid}")
            st.rerun()
        except Exception as exc:
            st.error(f"Customer save nahi hui: {exc}")

search = st.text_input("🔍 Customer Search karein")

try:
    df = get_customers_for_ui(search=search)
except Exception as exc:
    st.error(f"Database load nahi hua: {exc}")
    st.stop()
...
for row in df:
    with st.container(border=True):
        ...
        c2.metric("Sale", f"Rs. {float(row.get('total_sale', 0)):,.0f}")
        c3.metric("Outstanding", f"Rs. {float(row.get('outstanding', 0)):,.0f}")
