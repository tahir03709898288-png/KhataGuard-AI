    ...
    if save:
        try:
            if sale <= 0 and paid <= 0:
                raise ValueError("Sale ya payment mein se kam az kam ek amount enter karein.")
            
            # Flexible Logic: Allow paid > sale for debt clearance & advance payments
            if sale > 0:
...
st.divider()
st.subheader("📋 Saari Transactions")
